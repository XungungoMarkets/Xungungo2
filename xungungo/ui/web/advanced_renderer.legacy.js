...
(function (global) {
  ...
  // ========================================
  // Fibonacci Levels Primitive (horizontal lines with labels)
  // ========================================
  ...
  class FibonacciLevelsPrimitive {
    constructor(options) {
      this._opts = options || {};
      this._enabled = true;
      this._chart = null;
  ...
  // Extend AdvancedRenderer to handle fibonacci_levels
  const originalApply = AdvancedRenderer.prototype.apply;
  AdvancedRenderer.prototype.apply = function(chart, ensureLineSeriesFn, seriesDefs, indicators, candleSeries) {
    // Call original apply for other types
    originalApply.call(this, chart, ensureLineSeriesFn, seriesDefs, indicators, candleSeries);

    // Handle fibonacci_levels
    for (const def of seriesDefs) {
      if (def.type === "fibonacci_levels") {
        this._applyFibonacciLevels(chart, ensureLineSeriesFn, def, indicators, candleSeries);
      }
    }
  };

  AdvancedRenderer.prototype._applyFibonacciLevels = function(chart, ensureLineSeriesFn, def, indicators, candleSeries) {
    const id = def.id;
  ...
  global.FibonacciLevelsPrimitive = FibonacciLevelsPrimitive;
})(window);
